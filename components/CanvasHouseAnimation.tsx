...
const CanvasHouseAnimation = () => {
  ...
  useEffect(() => {
    ...
    // Set initial state
    let index = 0;
    const speed = 1; // Reduced from 2 to 1 - draw only one line per frame
    let lastDrawTime = 0;
    const drawInterval = 120; // Minimum time between drawings in ms (120ms = ~8 lines per second)
    ...
    const lines = [
      ...
      { x1: 200, y1: 220, x2: 200, y2: 190 },
      { x1: 175, y1: 205, x2: 225, y2: 205 },
      { x1: 400, y1: 220, x2: 400, y2: 190 },
      { x1: 375, y1: 205, x2: 425, y2: 205 },
      
      // Additional details
      { x1: 200, y1: 300, x2: 200, y2: 250 },
      { x1: 400, y1: 300, x2: 400, y2: 250 },
      { x1: 300, y1: 170, x2: 300, y2: 80 },
    ];
    
    // Ensure we have at least 60 steps for a 7+ second animation
    // Add some small details to extend the drawing process
    for (let i = 0; i < 20; i++) {
      // Small details like inner supports or decorative elements
    ...
    const animateSketch = (timestamp: number) => {
      ...
      if (index < lines.length) {
        ...
        // Restart animation after a longer delay
        setTimeout(() => {
          index = 0;
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          playAudio();
...
export default CanvasHouseAnimation; 